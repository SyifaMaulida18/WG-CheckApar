...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h2 text-dark">Edit Pengguna: {{ $user->name }}</h1>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="role" name="role" class="form-select" required>
                            ...
                            <option value="so" {{ old('role', $user->role) == 'so' ? 'selected' : '' }}>Safety Officer (SO)</option>
                        </select>
                        @error('role')<div class="text-danger mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6" id="subkon-group" style="display: {{ old('role', $user->role) == 'so' ? 'block' : 'none' }};">
                        <label for="subkon_nama" class="form-label">Nama Subkon</label>
                        <input type="text" class="form-control" id="subkon_nama" name="subkon_nama" value="{{ old('subkon_nama', $user->subkon_nama) }}">
