...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h2 text-dark">Tambah APAR Baru</h1>
        <a href="{{ route('admin.apars.index') }}" class="btn btn-secondary">Kembali</a>
    </div>

    <div class="card shadow-sm mb-4">
        <div class="card-body p-4">
            <form action="{{ route('admin.apars.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        <label for="nomor_seri" class="form-label">Nomor Seri</label>
                        <input type="text" class="form-control" id="nomor_seri" name="nomor_seri" value="{{ old('nomor_seri') }}" required>
                        @error('nomor_seri')<div class="text-danger mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label for="jenis_apar" class="form-label">Jenis APAR</label>
                        <input type="text" class="form-control" id="jenis_apar" name="jenis_apar" value="{{ old('jenis_apar') }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="kepemilikan" name="kepemilikan" required>
                            ...
                            <option value="Subkon" {{ old('kepemilikan') == 'Subkon' ? 'selected' : '' }}>Subkon</option>
                        </select>
                        @error('kepemilikan')<div class="text-danger mt-1">{{ $message }}</div>@enderror
                    </div>
                    
                    <div class="col-md-6" id="nama-subkon-group" style="display: {{ old('kepemilikan') == 'Subkon' ? 'block' : 'none' }};">
                        <label for="nama_subkon" class="form-label">Nama Subkon</label>
                        <input type="text" class="form-control" id="nama_subkon" name="nama_subkon" value="{{ old('nama_subkon') }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="gedung" name="gedung" required>
                            ...
                            <option value="gedung_g" {{ old('gedung') == 'gedung_g' ? 'selected' : '' }}>Gedung G</option>
                            <option value="gedung_f" {{ old('gedung') == 'gedung_f' ? 'selected' : '' }}>Gedung F</option>
                            <option value="gedung_h" {{ old('gedung') == 'gedung_h' ? 'selected' : '' }}>Gedung H</option>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="status_posisi" name="status_posisi" required>
                            ...
                            <option value="Permanen" {{ old('status_posisi') == 'Permanen' ? 'selected' : '' }}>Permanen</option>
                            <option value="Non-permanen" {{ old('status_posisi') == 'Non-permanen' ? 'selected' : '' }}>Non-permanen</option>
                        </select>
                    ...
                    <div class="col-12 mt-4">
                        ...
                        <div class="row g-3 mt-2">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="koordinat_x" name="koordinat_x" value="{{ old('koordinat_x') }}" readonly required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="koordinat_y" name="koordinat_y" value="{{ old('koordinat_y') }}" readonly required>
                    ...
                    <div class="col-12 text-end">
                        <button type="submit" class="btn btn-primary">Simpan APAR</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const statusPosisiSelect = document.getElementById('status_posisi'); // Tambahkan elemen ini
        ...
        const pinSvg = document.getElementById('pin-svg'); // Tambahkan elemen ini
        ...
        // Fungsi untuk mengubah warna pin berdasarkan status posisi
        function updatePinColor() {
            ...
            if (status === 'Permanen') {
                ...
                pinSvg.setAttribute('fill', 'grey'); // Warna default jika tidak ada yang dipilih
            }
        }
        ...
        function updateDenah() {
            ...
            if (gedung && lantai) {
                // Pastikan Anda memiliki gambar denah di folder public/denah/
        ...
        gedungSelect.addEventListener('change', updateDenah);
        lantaiSelect.addEventListener('change', updateDenah);
        statusPosisiSelect.addEventListener('change', updatePinColor); // Tambahkan event listener ini
        ...
        denahImage.addEventListener('click', function (e) {
            ...
            // Panggil fungsi updatePinColor saat pin ditempatkan
            updatePinColor();
